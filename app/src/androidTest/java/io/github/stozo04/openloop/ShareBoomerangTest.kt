...
class ShareBoomerangTest {
    ...
    @Test
    fun fileProvider_exposesVideoFile_asContentUri() {
        val videos = File(context.filesDir, "videos").apply { mkdirs() }
        val loopFile = File(videos, "boom_test_from_1.mp4").apply { writeBytes(ByteArray(4)) }
        try {
            val uri = FileProvider.getUriForFile(context, authority, loopFile)
            assertNotNull(uri)
    ...
    fun fileProvider_rejectsScratchCapture_notInConfiguredPaths() {
        ...
        try {
            assertThrows(IllegalArgumentException::class.java) {
                FileProvider.getUriForFile(context, authority, scratchFile)
            }
        } finally {
            scratchFile.delete()
        }
    ...
    fun buildBoomerangShareIntent_hasSendActionTypeStreamSubjectAndReadGrant() {
        val uri = Uri.parse("content://$authority/videos/boom_test_from_1.mp4")

        val intent = buildBoomerangShareIntent(uri, "OpenLoop boomerang")
        ...
        assertEquals("OpenLoop boomerang", intent.getStringExtra(Intent.EXTRA_SUBJECT))
        assertTrue(
