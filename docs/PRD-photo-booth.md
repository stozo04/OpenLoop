...
- Reentrancy guarded by the existing `photoSaveInProgress` flag (shared with photo capture — `captureBoothStrip` checks it directly).
...
| Layer             | Coverage                                                                                                                                                                                                                                                                                                                                                                                                      |
| ----------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| JVM unit          | `BoothStripLayout`: strip dimensions, frame rects, **top-biased square crop for tall 9:19.5 and wide sources** (assert the crop origin is not vertical-center on a tall source), footer band; `captureBoothStrip` saves + emits `Share`, guards reentrancy, rejects incomplete frame sets ([Lesson 008](./lessons_learned/008-jvm-test-file-and-dispatcher-pitfalls.md) — real temp dirs, shared dispatcher)  |
