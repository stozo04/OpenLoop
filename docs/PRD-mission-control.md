...
| 17  | **Two ML vendors: ML Kit for faces, MediaPipe (`tasks-vision`) for hands only** (`docs/PRD-lens-hand-flick.md` D2, 2026-08-26) | Amends PRD-camera-lenses §5.1's "don't ship both", which was about two *face* trackers. Every `LensAnchor` number is calibrated on ML Kit's landmarks; the hand verb needs a hand tracker ML Kit does not have. `tasks-core 1.0.0` measured 16 KB-aligned (Lesson 011) | +11 MB native + 7.8 MB model on arm64; consolidating faces onto MediaPipe Face Landmarker is backlog, not this change   |
| 18  | **No picture-in-picture** (Issue #150, 2026-08-27)                                                                             | Play's vitals card asks every video-playing app for PiP (71 % of peers). PiP keeps long-form/live video going; OpenLoop only plays 2–10 s silent loops in the gallery's in-screen `Dialog`, and the camera cannot leave the foreground. Nothing to keep watching       | Card stays on the overview — ignore it (triage: `docs/play-store/README.md`). Revisit if long-form playback ships       |
