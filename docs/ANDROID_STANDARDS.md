...
**Edge-to-edge is mandatory at target 36.** The `windowOptOutEdgeToEdgeEnforcement` opt-out is removed, so every screen must consume `WindowInsets` — no interactive element (shutter, home, gallery delete, onboarding arrows) may sit under the status/navigation bars or display cutout. `MainActivity` calls `enableEdgeToEdge()`, and every screen consumes insets (`safeDrawingPadding` on onboarding; `statusBarsPadding`/`navigationBarsPadding` on camera, gallery, trim, and the boomerang editor). The permission and loading screens are centered content that never reaches the bars. Play's vitals overview still shows two edge-to-edge "recommended actions"; both trace to `androidx.activity`'s own `enableEdgeToEdge()` internals and are triaged in [`play-store/README.md`](play-store/README.md#android-vitals-recommended-actions--standing-triage) (Issue #150) — do not migrate off `enableEdgeToEdge()` to clear them.
