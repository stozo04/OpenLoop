...
|---|---|---|---|
| W1 | **The warp centre is hardcoded to the mouth midpoint.** `WarpSpec` carries only `radiusInUnits` + `strength`. | `LensAnchor.warp()` L367–376; `WarpSpec` L153 | The catalogue can express exactly **one** warp today: a mouth-centred circle. Any other warp — including the ballot's Bug Eyes — is a §2.1 scope increase. Cheap fix: add an offset to `WarpSpec`, resolve it in `LensAnchor.warp`. Pure math, JVM-testable, `WarpCircle` and the shader untouched. ~10 lines + 2 tests. |
...
| C3 | **`LensPlacement` has no sideways term** — every sticker is locked to the face centre line. | `LensPlacement` L92; its KDoc says "add one when a lens actually needs to be off-centre" | An off-centre prop needs a `rightInUnits` field. ~4 lines + 1 test. |
| A1 | **Art over 1024 px on the long side is silently distorted. Live latent bug.** | `loadTexture()` L523–527 clamps width and height **independently** to `MAX_ART_PX = 1024`, then `setBounds(0,0,w,h)` rasterises into that box | A 1320×660 asset becomes a 1024×660 bitmap — content squashed 24% horizontally — while the quad still uses the declared `artAspect`. Broccoli never hit it (900×1005, under the cap on both axes). **Practical rule for this swarm: ship every art asset ≤1024 px on the long side.** |
...
- **Dated third-party coverage** — news, Know Your Meme, encyclopaedic entries with dates.
...
centre — i.e. W1 and nothing else. I nearly pitched it. I did **not**, and the reason is in §3.
...
**±0.4 units** from the centre line. That is the number the warp radii below are built on.
...
centre is hardcoded to the mouth. Bug Eyes needs it on the eye line.
...
**Art source + licence.** None. Warps carry no art. Carousel icon is an in-repo vector like
...
**Biggest risk + kill criterion.** One circle centred between the eyes also magnifies the nose
...
`delta *= 1.0 - uWarpStrength*falloff²`; at the centre `falloff = 1`, so a strength of `-0.55`
...
**Art source + licence.** None. In-repo vector carousel icon.
...
so a circle of radius 1.4 centred a third of a unit *below* the eye line — around the nose, the head's
...
device's `CLAMP_TO_EDGE` behaviour at the rim smears visibly, the effect looks like a bug rather
...
subject's own face fully visible between them. The single most recognisable AR effect ever shipped.

**Tier and why it suffices.** Prop — one alpha'd sticker over a still-visible face, exactly
...
**Art source + licence.** **Authored in-repo vector drawable**, same as `lens_sunglasses.xml` and
`lens_big_mouth.xml` — Apache 2.0, unambiguously ours, no attribution line, no redistribution
question. This is the cleanest licence story on my whole list, and it matters: PRD §11.2 records
that broccoli's photographic source needed an explicit owner licence confirmation before it could
...
2.30. Centre = (1.85 + −0.45)/2 = **0.70**. `artAspect` = 2.30/2.40 = **0.96** — to be re-measured
...
**Art source + licence — this is the weak leg.** We have exactly **one** owner-supplied character
image in this repo (`football.jpg`), and PRD §11.2 shows a photographic character asset needed an
explicit owner licence confirmation before it could ship. Steven is AFK, so **no new owner
...
licence-clean path. Not blocked — but it is real work with a real failure mode, and it is the only
candidate on my list whose licence is not settled by construction.
...
centre than Broccoli's because the "face" is a flat disc rather than a recessed opening.
...
an asset with an unresolved licence, and I cannot get an owner confirmation this run.
...
**Art source + licence.** Authored in-repo vector. Apache 2.0, no attribution.
...
span 0.80, centre (1.25 + 2.05)/2 = 1.65; aspect 0.80/1.80 = 0.44.
...
tier, the same cost, the same licence story, and actually funny.** Kill criterion: if we have a
better prop, this dies — and we do.

---

### C6 — Crying-Laughing Emoji Head · **character** · the licence-clean alternative to C4
...
**Art source + licence — this is why it exists on my list.** **Authored in-repo vector.** A
...
CC-BY-SA, and Noto's licensing is a font-licence question I do not want in an Apache 2.0 app. Drawing
it ourselves sidesteps all three. **This is the only character candidate whose licence is settled by
...
| Rejected | Why |
|---|---|
| **Balloon / Inflate Face** (the live 2026 trend from §1c) | Mechanically it is our shipped Big Mouth with a larger radius and a shifted centre — the *same* joke (inflate a face) at a different scale. GOAL §2.3.2 kills "another mouth bulge". C2 (Tiny Head) gets the same trend energy from the **opposite sign**, which is genuinely a different lens. Pitching both would be pitching one idea twice. If C2 dies, this is my backup. |
...
4. **The fourth slot is the genuinely open question**, and I am not going to pretend otherwise.
   C6 wins on licence (settled by construction, no sourcing risk, no absent owner needed) and on
...
§4.1 table that is *exactly* tight at the top: centred at `upInUnits = -0.35`, a 1.60 half-width
